...
-- Conventions: <root>_id BIGINT IDENTITY PK; <root>_no VARCHAR(30) UNIQUE;
...
CREATE TABLE sec_user (
    ...
    email         VARCHAR(150),
    employee_id   BIGINT,                 -- FK md_employee (deferred)
    home_site_id  BIGINT,                 -- FK md_location (deferred)
...
CREATE TABLE sec_user_role (
    ...
    role_id      BIGINT NOT NULL REFERENCES sec_role(role_id),
    created_by   BIGINT NOT NULL REFERENCES sec_user(user_id),
    created_at   TIMESTAMPTZ NOT NULL DEFAULT now(),
    is_active    BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE TABLE sec_role_permission (
    ...
    role_id       BIGINT NOT NULL REFERENCES sec_role(role_id),
    permission_id BIGINT NOT NULL REFERENCES sec_permission(permission_id),
    created_by    BIGINT NOT NULL REFERENCES sec_user(user_id),
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    is_active     BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE TABLE sys_setting (
    ...
    description   VARCHAR(300),
    site_id       BIGINT,                  -- null = global (FK md_location deferred)
    created_by    BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by    BIGINT      REFERENCES sec_user(user_id),
    updated_at    TIMESTAMPTZ,
...
CREATE TABLE md_location (
    ...
    parent_location_id BIGINT       REFERENCES md_location(location_id),
    address            VARCHAR(300),
    created_by         BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at         TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by         BIGINT      REFERENCES sec_user(user_id),
...
CREATE TABLE md_item_category (
    ...
    parent_category_id BIGINT       REFERENCES md_item_category(category_id),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by BIGINT      REFERENCES sec_user(user_id),
    updated_at TIMESTAMPTZ,
...
CREATE TABLE md_department (
    ...
    parent_department_id BIGINT  REFERENCES md_department(department_id),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by BIGINT      REFERENCES sec_user(user_id),
    updated_at TIMESTAMPTZ,
...
CREATE TABLE md_item (
    ...
    category_id       BIGINT       REFERENCES md_item_category(category_id),
    group_id          BIGINT       REFERENCES md_item_group(group_id),
    base_uom_id       BIGINT       NOT NULL REFERENCES md_uom(uom_id),
    ...
    default_supplier_id BIGINT     REFERENCES md_supplier(supplier_id),
    barcode           VARCHAR(50),
    tax_code          VARCHAR(20),
...
CREATE TABLE md_uom_conversion (
    ...
    item_id       BIGINT       REFERENCES md_item(item_id),   -- null = global conversion
    from_uom_id   BIGINT NOT NULL REFERENCES md_uom(uom_id),
    to_uom_id     BIGINT NOT NULL REFERENCES md_uom(uom_id),
...
CREATE TABLE md_asset (
    ...
    site_id         BIGINT       NOT NULL REFERENCES md_location(location_id),
    department_id   BIGINT       REFERENCES md_department(department_id),
    cost_center_id  BIGINT       REFERENCES md_cost_center(cost_center_id),
...
CREATE TABLE md_asset_vehicle (
    asset_id     BIGINT PRIMARY KEY REFERENCES md_asset(asset_id),
...
CREATE TABLE md_employee (
    ...
    grade_id      BIGINT       REFERENCES md_employee_grade(grade_id),
    department_id BIGINT       REFERENCES md_department(department_id),
    site_id       BIGINT       NOT NULL REFERENCES md_location(location_id),
...
CREATE TABLE md_labour_rate (
    labour_rate_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    grade_id       BIGINT NOT NULL REFERENCES md_employee_grade(grade_id),
...
CREATE TABLE md_warranty_term (
    warranty_term_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE md_approval_role (
    approval_role_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    role_code   VARCHAR(30)  NOT NULL,
    role_name   VARCHAR(100) NOT NULL,
    sec_role_id BIGINT       REFERENCES sec_role(role_id),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by BIGINT      REFERENCES sec_user(user_id),
    updated_at TIMESTAMPTZ,
...
CREATE TABLE md_battery (
    battery_id         BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    battery_serial_no  VARCHAR(50)  NOT NULL,
    item_id            BIGINT NOT NULL REFERENCES md_item(item_id),   -- battery model
    manufacturer       VARCHAR(80),
    capacity_ah        NUMERIC(10,2),
    voltage            NUMERIC(6,2),
    manufacture_date   DATE,
    purchase_grn_id    BIGINT,                 -- FK tx_grn (deferred)
    purchase_date      DATE,
    acquisition_cost   NUMERIC(18,2) NOT NULL DEFAULT 0,
    warranty_term_id   BIGINT REFERENCES md_warranty_term(warranty_term_id),
    warranty_start_date DATE,
    warranty_end_date   DATE,
    original_asset_id  BIGINT REFERENCES md_asset(asset_id),   -- first asset installed on
    current_asset_id   BIGINT REFERENCES md_asset(asset_id),   -- current asset (null in stock)
    current_location_id BIGINT REFERENCES md_location(location_id),
    battery_status     VARCHAR(22) NOT NULL DEFAULT 'IN_STOCK',
    install_meter      NUMERIC(18,2),
    last_event_id      BIGINT,                 -- FK hist_battery_event (deferred)
    site_id            BIGINT NOT NULL REFERENCES md_location(location_id),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by BIGINT      REFERENCES sec_user(user_id),
...
CREATE TABLE md_price_history (
    ...
    supplier_id     BIGINT REFERENCES md_supplier(supplier_id),
    site_id         BIGINT REFERENCES md_location(location_id),
...
CREATE TABLE md_price (
    ...
    effective_date  DATE          NOT NULL,
    price_status    VARCHAR(12)   NOT NULL DEFAULT 'CONFIRMED',
    price_history_id BIGINT REFERENCES md_price_history(price_history_id),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by BIGINT      REFERENCES sec_user(user_id),
    updated_at TIMESTAMPTZ,
...
CREATE TABLE sys_workflow_step (
    ...
    workflow_id      BIGINT NOT NULL REFERENCES sys_workflow(workflow_id),
    step_no          SMALLINT NOT NULL,
    step_name        VARCHAR(100) NOT NULL,
    approval_role_id BIGINT REFERENCES md_approval_role(approval_role_id),
    required_role_id BIGINT REFERENCES sec_role(role_id),
...
CREATE TABLE mv_stock_ledger (
    ...
    item_id              BIGINT NOT NULL REFERENCES md_item(item_id),
    location_id          BIGINT NOT NULL REFERENCES md_location(location_id),
    ...
    battery_id           BIGINT REFERENCES md_battery(battery_id),
    valuation_layer_id   BIGINT,                      -- FK inv_valuation_layer (deferred)
    is_provisional       BOOLEAN NOT NULL DEFAULT FALSE,
    reval_of_ledger_id   BIGINT REFERENCES mv_stock_ledger(ledger_id),
    posted_by            BIGINT NOT NULL REFERENCES sec_user(user_id),
...
CREATE TABLE inv_stock_balance (
    ...
    last_movement_id BIGINT REFERENCES mv_stock_ledger(ledger_id),
...
CREATE TABLE inv_valuation_layer (
    ...
    item_id         BIGINT NOT NULL REFERENCES md_item(item_id),
    location_id     BIGINT NOT NULL REFERENCES md_location(location_id),
    receipt_ledger_id BIGINT NOT NULL REFERENCES mv_stock_ledger(ledger_id),
...
CREATE TABLE inv_reservation (
    ...
    source_doc_id   BIGINT      NOT NULL,
    source_line_id  BIGINT,
    jobcard_id      BIGINT,                 -- FK tx_jobcard (deferred)
...
CREATE TABLE inv_pending_price (
    ...
    grn_id          BIGINT NOT NULL,        -- FK tx_grn (deferred)
    grn_line_id     BIGINT NOT NULL,        -- FK txl_grn (deferred)
    item_id         BIGINT NOT NULL REFERENCES md_item(item_id),
    location_id     BIGINT NOT NULL REFERENCES md_location(location_id),
    ...
    confirmed_price_history_id BIGINT REFERENCES md_price_history(price_history_id),
    reval_ledger_id BIGINT REFERENCES mv_stock_ledger(ledger_id),
    variance_amt    NUMERIC(18,2),
    resolved_by     BIGINT REFERENCES sec_user(user_id),
...
CREATE TABLE inv_lube_monthly_balance (
    lube_bal_id   BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    item_id       BIGINT NOT NULL REFERENCES md_item(item_id),
    location_id   BIGINT NOT NULL REFERENCES md_location(location_id),
...
CREATE TABLE tx_mrn (
    mrn_id        BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    mrn_no        VARCHAR(30) NOT NULL,
    mrn_date      DATE NOT NULL,
    location_id   BIGINT NOT NULL REFERENCES md_location(location_id),  -- requesting store
    department_id BIGINT REFERENCES md_department(department_id),
    asset_id      BIGINT REFERENCES md_asset(asset_id),
    jobcard_id    BIGINT,                 -- FK tx_jobcard (deferred)
    required_date DATE,
    priority      VARCHAR(10) NOT NULL DEFAULT 'NORMAL',
    doc_status    VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    requested_by  BIGINT REFERENCES md_employee(employee_id),
    remarks       VARCHAR(300),
    approved_by BIGINT      REFERENCES sec_user(user_id),
...
CREATE TABLE tx_po (
    ...
    supplier_id  BIGINT NOT NULL REFERENCES md_supplier(supplier_id),
    location_id  BIGINT NOT NULL REFERENCES md_location(location_id),  -- deliver-to store
    mrn_id       BIGINT REFERENCES tx_mrn(mrn_id),
    delivery_date DATE,
    currency_code CHAR(3) NOT NULL DEFAULT 'LKR',
...
CREATE TABLE txl_po (
    ...
    po_id        BIGINT NOT NULL REFERENCES tx_po(po_id),
    line_no      SMALLINT NOT NULL,
    item_id      BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id       BIGINT NOT NULL REFERENCES md_uom(uom_id),
    ...
    expected_date DATE,
    mrn_line_id  BIGINT REFERENCES txl_mrn(mrn_line_id),
    line_status  VARCHAR(15) NOT NULL DEFAULT 'OPEN',
    remarks      VARCHAR(200),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE tx_grn (
    ...
    po_id         BIGINT REFERENCES tx_po(po_id),
    supplier_id   BIGINT NOT NULL REFERENCES md_supplier(supplier_id),
    location_id   BIGINT NOT NULL REFERENCES md_location(location_id),  -- receiving store
    ...
    doc_status    VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    received_by   BIGINT REFERENCES md_employee(employee_id),
    remarks       VARCHAR(300),
    approved_by BIGINT      REFERENCES sec_user(user_id),
    approved_at TIMESTAMPTZ,
    site_id     BIGINT      NOT NULL REFERENCES md_location(location_id),
...
CREATE TABLE txl_grn (
    ...
    grn_id       BIGINT NOT NULL REFERENCES tx_grn(grn_id),
    line_no      SMALLINT NOT NULL,
    item_id      BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id       BIGINT NOT NULL REFERENCES md_uom(uom_id),
    location_id  BIGINT REFERENCES md_location(location_id),   -- bin
    ...
    po_line_id   BIGINT REFERENCES txl_po(po_line_id),
...
CREATE TABLE tx_issue (
    ...
    location_id   BIGINT NOT NULL REFERENCES md_location(location_id),  -- from store
    ...
    mrn_id        BIGINT REFERENCES tx_mrn(mrn_id),
    requested_by  BIGINT REFERENCES md_employee(employee_id),
    issued_to_employee_id BIGINT REFERENCES md_employee(employee_id),
    total_amt     NUMERIC(18,2) NOT NULL DEFAULT 0,
    doc_status    VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    override_by   BIGINT REFERENCES sec_user(user_id),
...
CREATE TABLE txl_issue (
    ...
    issue_id      BIGINT NOT NULL REFERENCES tx_issue(issue_id),
    line_no       SMALLINT NOT NULL,
    item_id       BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id        BIGINT NOT NULL REFERENCES md_uom(uom_id),
...
CREATE TABLE tx_transfer (
    ...
    from_location_id BIGINT NOT NULL REFERENCES md_location(location_id),
    to_location_id   BIGINT NOT NULL REFERENCES md_location(location_id),
    from_site_id    BIGINT NOT NULL REFERENCES md_location(location_id),
    to_site_id      BIGINT NOT NULL REFERENCES md_location(location_id),
...
CREATE TABLE txl_transfer (
    ...
    transfer_id   BIGINT NOT NULL REFERENCES tx_transfer(transfer_id),
    line_no       SMALLINT NOT NULL,
    item_id       BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id        BIGINT NOT NULL REFERENCES md_uom(uom_id),
    ...
    line_amt      NUMERIC(18,2) NOT NULL DEFAULT 0,
    batch_no      VARCHAR(40),
    out_ledger_id BIGINT REFERENCES mv_stock_ledger(ledger_id),
    in_ledger_id  BIGINT REFERENCES mv_stock_ledger(ledger_id),
    remarks       VARCHAR(200),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_by BIGINT      REFERENCES sec_user(user_id),
...
CREATE TABLE txl_adjustment (
    adjustment_line_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    adjustment_id BIGINT NOT NULL REFERENCES tx_adjustment(adjustment_id),
    line_no       SMALLINT NOT NULL,
    item_id       BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id        BIGINT NOT NULL REFERENCES md_uom(uom_id),
...
CREATE TABLE tx_return (
    ...
    supplier_id  BIGINT REFERENCES md_supplier(supplier_id),
    location_id  BIGINT NOT NULL REFERENCES md_location(location_id),
    grn_id       BIGINT REFERENCES tx_grn(grn_id),
    total_amt    NUMERIC(18,2) NOT NULL DEFAULT 0,
    doc_status   VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    remarks      VARCHAR(300),
    approved_by BIGINT      REFERENCES sec_user(user_id),
...
CREATE TABLE txl_return (
    ...
    return_id     BIGINT NOT NULL REFERENCES tx_return(return_id),
    line_no       SMALLINT NOT NULL,
    item_id       BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id        BIGINT NOT NULL REFERENCES md_uom(uom_id),
    ...
    grn_line_id   BIGINT REFERENCES txl_grn(grn_line_id),
    ledger_id     BIGINT REFERENCES mv_stock_ledger(ledger_id),
    reason        VARCHAR(200),
    created_by BIGINT      NOT NULL REFERENCES sec_user(user_id),
    created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
...
CREATE TABLE tx_lube_issue (
    lube_issue_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE txl_lube_issue (
    lube_line_id  BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    lube_issue_id BIGINT NOT NULL REFERENCES tx_lube_issue(lube_issue_id),
    line_no       SMALLINT NOT NULL,
    item_id       BIGINT NOT NULL REFERENCES md_item(item_id),   -- item_type = LUBRICANT
    uom_id        BIGINT NOT NULL REFERENCES md_uom(uom_id),
...
CREATE TABLE tx_battery_issue (
    battery_issue_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    battery_issue_no VARCHAR(30) NOT NULL,
    issue_date       DATE NOT NULL,
    battery_id       BIGINT NOT NULL REFERENCES md_battery(battery_id),
    item_id          BIGINT NOT NULL REFERENCES md_item(item_id),
    from_location_id BIGINT NOT NULL REFERENCES md_location(location_id),
    to_asset_id      BIGINT NOT NULL REFERENCES md_asset(asset_id),
    meter_reading    NUMERIC(18,2),
    installed_by_employee_id BIGINT REFERENCES md_employee(employee_id),
    jobcard_id       BIGINT,                 -- FK tx_jobcard (deferred)
    warranty_start_date DATE,
    battery_status_after VARCHAR(22) NOT NULL DEFAULT 'IN_SERVICE',
    ledger_id        BIGINT REFERENCES mv_stock_ledger(ledger_id),  -- OUT of serialized stock
    hist_event_id    BIGINT,                 -- FK hist_battery_event (deferred)
    doc_status       VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    remarks          VARCHAR(300),
    approved_by BIGINT      REFERENCES sec_user(user_id),
    approved_at TIMESTAMPTZ,
...
CREATE TABLE tx_battery_transfer (
    battery_transfer_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    battery_transfer_no VARCHAR(30) NOT NULL,
    transfer_date  DATE NOT NULL,
    battery_id     BIGINT NOT NULL REFERENCES md_battery(battery_id),
    from_asset_id  BIGINT REFERENCES md_asset(asset_id),
    to_asset_id    BIGINT NOT NULL REFERENCES md_asset(asset_id),
...
CREATE TABLE tx_battery_return (
    battery_return_id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    replacement_battery_id BIGINT REFERENCES md_battery(battery_id),
    warranty_term_id BIGINT REFERENCES md_warranty_term(warranty_term_id),
    warranty_claim_no VARCHAR(40),
    condition_note VARCHAR(200),
    meter_reading  NUMERIC(18,2),
    battery_status_after VARCHAR(22) NOT NULL,
    ledger_id      BIGINT REFERENCES mv_stock_ledger(ledger_id),  -- IN to stock if returned
    hist_event_id  BIGINT,                 -- FK hist_battery_event (deferred)
    doc_status     VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    remarks        VARCHAR(300),
    approved_by BIGINT      REFERENCES sec_user(user_id),
...
CREATE TABLE hist_battery_event (
    ...
    from_asset_id  BIGINT REFERENCES md_asset(asset_id),
    to_asset_id    BIGINT REFERENCES md_asset(asset_id),
    from_location_id BIGINT REFERENCES md_location(location_id),
    to_location_id   BIGINT REFERENCES md_location(location_id),
    meter_reading  NUMERIC(18,2),
...
CREATE TABLE tx_jobcard (
    jobcard_id     BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    ...
    asset_id       BIGINT NOT NULL REFERENCES md_asset(asset_id),
    location_id    BIGINT NOT NULL REFERENCES md_location(location_id),  -- workshop
    job_type       VARCHAR(14) NOT NULL DEFAULT 'BREAKDOWN', -- BREAKDOWN|PREVENTIVE|ACCIDENT|RUNNING_REPAIR|INSPECTION|GENERAL
    priority       VARCHAR(10) NOT NULL DEFAULT 'NORMAL',
    reported_by_employee_id BIGINT REFERENCES md_employee(employee_id),
    reported_defect TEXT,
    meter_reading  NUMERIC(18,2),
    meter_type     VARCHAR(4),
    assigned_to_employee_id BIGINT REFERENCES md_employee(employee_id),
    project_id     BIGINT REFERENCES md_project(project_id),
    department_id  BIGINT REFERENCES md_department(department_id),
    cost_center_id BIGINT REFERENCES md_cost_center(cost_center_id),
    ...
    doc_status     VARCHAR(15) NOT NULL DEFAULT 'DRAFT',
    tm_approved_by BIGINT REFERENCES sec_user(user_id),
    tm_approved_at TIMESTAMPTZ,
    om_approved_by BIGINT REFERENCES sec_user(user_id),
...
CREATE TABLE tx_job_progress (
    ...
    jobcard_id     BIGINT NOT NULL REFERENCES tx_jobcard(jobcard_id),
    task_id        BIGINT REFERENCES txl_jobcard_task(task_id),
    progress_date  DATE NOT NULL,
    work_done      TEXT NOT NULL,
    pct_complete   NUMERIC(5,2) NOT NULL DEFAULT 0,
    hours_spent    NUMERIC(9,2) NOT NULL DEFAULT 0,
    logged_by_employee_id BIGINT REFERENCES md_employee(employee_id),
...
CREATE TABLE tx_job_material_req (
    mrq_id       BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    mrq_no       VARCHAR(30) NOT NULL,
    mrq_date     DATE NOT NULL,
    jobcard_id   BIGINT NOT NULL REFERENCES tx_jobcard(jobcard_id),
    location_id  BIGINT NOT NULL REFERENCES md_location(location_id),  -- store
...
CREATE TABLE txl_job_material_req (
    mrq_line_id  BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    mrq_id       BIGINT NOT NULL REFERENCES tx_job_material_req(mrq_id),
    line_no      SMALLINT NOT NULL,
    item_id      BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id       BIGINT NOT NULL REFERENCES md_uom(uom_id),
    task_id      BIGINT REFERENCES txl_jobcard_task(task_id),
...
CREATE TABLE tx_job_parts (
    job_part_id  BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    jobcard_id   BIGINT NOT NULL REFERENCES tx_jobcard(jobcard_id),
    task_id      BIGINT REFERENCES txl_jobcard_task(task_id),
    item_id      BIGINT NOT NULL REFERENCES md_item(item_id),
    uom_id       BIGINT NOT NULL REFERENCES md_uom(uom_id),
    ...
    source_doc_id   BIGINT,
    source_line_id  BIGINT,
    issue_line_id BIGINT REFERENCES txl_issue(issue_line_id),
    mrq_id        BIGINT REFERENCES tx_job_material_req(mrq_id),
    ledger_id     BIGINT REFERENCES mv_stock_ledger(ledger_id),
...
CREATE TABLE tx_job_labour (
    ...
    jobcard_id   BIGINT NOT NULL REFERENCES tx_jobcard(jobcard_id),
    task_id      BIGINT REFERENCES txl_jobcard_task(task_id),
    employee_id  BIGINT NOT NULL REFERENCES md_employee(employee_id),
    ...
    grade_id     BIGINT REFERENCES md_employee_grade(grade_id),
    labour_rate_id BIGINT REFERENCES md_labour_rate(labour_rate_id),
...
CREATE TABLE tx_job_outside_repair (
    osr_id       BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    osr_no       VARCHAR(30) NOT NULL,
    jobcard_id   BIGINT NOT NULL REFERENCES tx_jobcard(jobcard_id),
    task_id      BIGINT REFERENCES txl_jobcard_task(task_id),
    subcontractor_id BIGINT NOT NULL REFERENCES md_supplier(supplier_id),
    description  TEXT,
    sent_date    DATE,
...
CREATE TABLE cost_job_summary (
    ...
    finalized_by  BIGINT REFERENCES sec_user(user_id),
...
CREATE TABLE cost_job_line (
    ...
    jobcard_id   BIGINT NOT NULL REFERENCES tx_jobcard(jobcard_id),
    summary_id   BIGINT REFERENCES cost_job_summary(summary_id),
    cost_element VARCHAR(10) NOT NULL,   -- MATERIAL|LABOUR|OUTSIDE|GENERAL|OVERHEAD
    source_doc_type VARCHAR(10),
    source_doc_id   BIGINT,
    source_line_id  BIGINT,
    item_id      BIGINT REFERENCES md_item(item_id),
    employee_id  BIGINT REFERENCES md_employee(employee_id),
    task_id      BIGINT REFERENCES txl_jobcard_task(task_id),
...
CREATE TABLE apr_request (
    ...
    amount         NUMERIC(18,2),
    requested_by   BIGINT NOT NULL REFERENCES sec_user(user_id),
...
CREATE TABLE apr_step (
    apr_step_id    BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    request_id     BIGINT NOT NULL REFERENCES apr_request(request_id),
    step_no        SMALLINT NOT NULL,
    workflow_step_id BIGINT REFERENCES sys_workflow_step(step_id),
    assigned_role_id BIGINT REFERENCES sec_role(role_id),
    assigned_user_id BIGINT REFERENCES sec_user(user_id),
    step_status    VARCHAR(12) NOT NULL DEFAULT 'PENDING',
    acted_by       BIGINT REFERENCES sec_user(user_id),
...
CREATE TABLE apr_action (
    ...
    request_id   BIGINT NOT NULL REFERENCES apr_request(request_id),
    apr_step_id  BIGINT NOT NULL REFERENCES apr_step(apr_step_id),
    action_type  VARCHAR(10) NOT NULL,   -- SUBMIT|APPROVE|REJECT|RETURN|ESCALATE|DELEGATE
    action_by    BIGINT NOT NULL REFERENCES sec_user(user_id),
    action_at    TIMESTAMPTZ NOT NULL DEFAULT now(),
    from_status  VARCHAR(12),
    to_status    VARCHAR(12),
    delegated_to BIGINT REFERENCES sec_user(user_id),
...
ALTER TABLE sec_user
    ADD CONSTRAINT fk_sec_user_employee   FOREIGN KEY (employee_id)  REFERENCES md_employee(employee_id),
    ADD CONSTRAINT fk_sec_user_home_site  FOREIGN KEY (home_site_id) REFERENCES md_location(location_id);

ALTER TABLE sys_setting
    ADD CONSTRAINT fk_sys_setting_site    FOREIGN KEY (site_id)      REFERENCES md_location(location_id);

ALTER TABLE md_battery
    ADD CONSTRAINT fk_md_battery_grn      FOREIGN KEY (purchase_grn_id) REFERENCES tx_grn(grn_id),
    ADD CONSTRAINT fk_md_battery_last_evt FOREIGN KEY (last_event_id)   REFERENCES hist_battery_event(event_id);

ALTER TABLE mv_stock_ledger
    ADD CONSTRAINT fk_mv_ledger_val_layer FOREIGN KEY (valuation_layer_id) REFERENCES inv_valuation_layer(layer_id);

ALTER TABLE inv_reservation
    ADD CONSTRAINT fk_inv_reservation_jc  FOREIGN KEY (jobcard_id)   REFERENCES tx_jobcard(jobcard_id);

ALTER TABLE inv_pending_price
    ADD CONSTRAINT fk_inv_pending_grn     FOREIGN KEY (grn_id)       REFERENCES tx_grn(grn_id),
    ADD CONSTRAINT fk_inv_pending_grnline FOREIGN KEY (grn_line_id)  REFERENCES txl_grn(grn_line_id);

ALTER TABLE tx_mrn
    ADD CONSTRAINT fk_tx_mrn_jobcard      FOREIGN KEY (jobcard_id)   REFERENCES tx_jobcard(jobcard_id);

ALTER TABLE tx_issue
    ADD CONSTRAINT fk_tx_issue_jobcard    FOREIGN KEY (jobcard_id)   REFERENCES tx_jobcard(jobcard_id);

ALTER TABLE tx_lube_issue
    ADD CONSTRAINT fk_tx_lube_jobcard     FOREIGN KEY (jobcard_id)   REFERENCES tx_jobcard(jobcard_id);

ALTER TABLE tx_battery_issue
    ADD CONSTRAINT fk_tx_bat_issue_jc     FOREIGN KEY (jobcard_id)   REFERENCES tx_jobcard(jobcard_id),
    ADD CONSTRAINT fk_tx_bat_issue_evt    FOREIGN KEY (hist_event_id) REFERENCES hist_battery_event(event_id);

ALTER TABLE tx_battery_transfer
    ADD CONSTRAINT fk_tx_bat_xfer_evt     FOREIGN KEY (hist_event_id) REFERENCES hist_battery_event(event_id);

ALTER TABLE tx_battery_return
    ADD CONSTRAINT fk_tx_bat_ret_evt      FOREIGN KEY (hist_event_id) REFERENCES hist_battery_event(event_id);
