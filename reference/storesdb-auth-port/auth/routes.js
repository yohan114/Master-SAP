// Public auth routes for the storesdb app: login (rate-limited + lockout) and logout.
...
router.post('/login', loginLimiter, async (req, res) => {
  ...
  if (user.mfa_enabled) {                                   // second factor for MFA-enabled accounts
    if (!verifyTotp(user.mfa_secret, (req.body || {}).token)) {
  ...
  res.cookie('umms_sid', token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: SESSION_MS });
