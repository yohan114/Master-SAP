...
- [x] **Site‑scoped visibility** — **wired & proved across every read**: `site_id` on the stores tables (backfilled to the home site; child rows inherit the parent's site), `/api/*` reads require a session, and **all** reads — lists, single‑record `/:id` lookups, dropdowns, and dashboards/aggregates (`/api/dashboard/*`, `/api/sidebar-stats`, `/api/inventory`, `*/stats`) — filter by the user's `sec_user_site` (admin/`READ.ALL_SITES` see all). Verified end‑to‑end on the real `inventory.db` with a 2nd site: **20/20** core isolation + regression and **29/29** dashboard/aggregate sweep (site‑2 keeper: 10 items, 4 issues, ≤3 batteries, 2 transfers, 5 general SKUs; no read 500s). **Write side is guarded too** — `enforceSite` blocks a site‑restricted user from updating/deleting/moving another site's row by id (path `:id` and body‑id routes; **12/12** verified). *(Also fixed a pre‑existing legacy bug where `/api/inventory` always 500'd on an escaped `${cte}` template.)* *Remaining multi‑site step: stamp the creator's site on new rows (today they take the home‑site default — correct for a single‑site deployment).*

### Transport & secrets
- [~] **HTTPS/TLS** everywhere via reverse proxy — **config provided** (`ops/proxy/nginx-umms.conf`: TLS1.2/1.3, HSTS, CSP, redirect); *deploy + install certs.* **Go‑live kit added: `deploy/` (Docker compose app+nginx, systemd unit, `apply-port.sh`, `RUNBOOK.md`) — follow the runbook to install certs (certbot) and bring it up.** *(Owner: ___)*
- [~] Secrets removed from source/committed `.env` → OS secret store / vault; rotate all creds that ever touched code (incl. `E&CWorkshop`). **Code side confirmed clean**: scanned the shipped code — **no hardcoded secrets**, config is fully env‑driven (`INVENTORY_DB`, `NODE_ENV`, `HOME_SITE_ID`, `CORS_ORIGINS`), session tokens/TOTP secrets are random per‑use (no static secret to leak), the only tracked env file is a placeholder `.env.example`, and `.gitignore` blocks `.env`/`*.db`/`*.xlsx`/operational data. *Remaining (deploy): load real creds from a vault and **rotate** everything that ever touched code, including `E&CWorkshop`.* *(Owner: ___)*
- [x] CORS allow‑list, security headers, and request rate limiting — **app CORS allow‑list wired** (`reference/storesdb-auth-port/auth/cors.js`): env‑driven `CORS_ORIGINS`, **same‑origin only by default**, credentials‑safe (echoes the exact origin, never `*`), preflight → 204 (allowed) / 403 (disallowed). Verified 6/6 (allowed echoes ACAO, disallowed/no‑env → no ACAO, preflight allow/deny). Login rate‑limit + lockout already in the auth module; nginx config carries HSTS/CSP + a login rate‑limit zone. *(Deploy: front with the nginx config for edge headers.)*
- [x] Confirm all SQL is parameterized; keep `ORDER BY`/column names **allow‑listed** — **audited the whole app**: every value is a bound `?` param; the only interpolated identifiers are hardcoded column literals (the receipt `SET` builder maps request keys to fixed column names, values bound) and the sort columns, which are allow‑listed (`ITEM_SORTS`, `validSortCols`) with `order` normalized to `ASC`/`DESC`; no user input reaches a raw `exec()`; bulk import uses prepared statements. **The site‑scope helpers (`scopeWhere`/`andSite`/`whereSite`) preserve this** (fixed alias + `?` placeholders + bound ids). *Found & fixed one gap: `/api/export/excel` was an unscoped bulk read — now site‑scoped like the rest (unit‑verified: site‑2 keeper exports 10 items vs admin's 4000).*
