...
- [~] **Site‑scoped visibility** — **wired & proved** on the core list reads: `site_id` added to the stores tables (backfilled to the home site), `/api/*` reads now require a session, and `/api/items|issues|transfers|general-items|batteries` filter by the user's `sec_user_site` (admin/`READ.ALL_SITES` see all). Verified end‑to‑end on the real `inventory.db` with a 2nd site — **20/20** (admin=totals, site‑1 keeper=site‑1 only, site‑2 keeper=site‑2 only). *Remaining: apply the same one‑line `scopeWhere(req)` splice to the dashboard/aggregate endpoints (`/api/dashboard/*`, `/api/inventory`, `*/stats`) before exposing them to site‑restricted users.* *(Owner: ___)*
